# Databricks notebook source exported at Mon, 4 Jul 2016 21:26:35 UTC
# MAGIC %md
...
days_with_hosts,hosts = [list(a) for a in zip(*daily_hosts_df.select('day', 'count').map(lambda r: (r[0], r[1])).collect())]
...
avg_daily_req_per_host_df = (
  total_req_per_day_df.join(
    daily_hosts_df, daily_hosts_df['day']==total_req_per_day_df['day']
  ).drop(daily_hosts_df['day']).select(
...
days_with_avg,avgs = [list(a) for a in zip(*avg_daily_req_per_host_df.select('day', 'avg_reqs_per_host_per_day').map(lambda r: (r[0], r[1])).collect())]
...
# TODO: Replace <FILL IN> with appropriate code
days_with_errors_404,errors_404_by_day = [list(a) for a in zip(*errors_by_date_sorted_df.select('day', 'count').map(lambda r: (r[0], r[1])).collect())]
...
# TODO: Replace <FILL IN> with appropriate code
hours_with_not_found,not_found_counts_per_hour = [list(a) for a in zip(*hour_records_sorted_df.select('hour', 'count').map(lambda r: (r[0], r[1])).collect())]
